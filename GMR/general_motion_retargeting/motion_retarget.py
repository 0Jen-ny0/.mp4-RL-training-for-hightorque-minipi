...
class GeneralMotionRetargeting:
    ...
    def offset_human_data(self, human_data, pos_offsets, rot_offsets):
        """Apply offsets; allow missing keys (SMPLX has more joints than our IK table)."""
        offset_human_data = {}

        identity = R.from_quat([1, 0, 0, 0], scalar_first=True)

        for body_name in human_data.keys():
            pos, quat = human_data[body_name]

            # default offsets if not specified
            rot_off = rot_offsets.get(body_name, identity)
            pos_off = np.array(pos_offsets.get(body_name, [0.0, 0.0, 0.0]))

            # apply rotation offset first
            updated_quat = (R.from_quat(quat, scalar_first=True) * rot_off).as_quat(scalar_first=True)

            # compute global pos offset using updated rotation
            global_pos_offset = R.from_quat(updated_quat, scalar_first=True).apply(pos_off)
